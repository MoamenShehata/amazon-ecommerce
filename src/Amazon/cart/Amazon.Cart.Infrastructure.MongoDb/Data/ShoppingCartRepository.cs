...
namespace Amazon.Cart.Infrastructure.MongoDb.Data;

public class ShoppingCartRepository(IConfiguration configuration) : IRepository<ShoppingCart, Guid>
{
    private readonly MongoClient _client = new(configuration.GetConnectionString("Mongo"));

    public void Add(ShoppingCart aggregate)
    {
        var collection = _client.GetDatabase("amazon-cart").GetCollection<ShoppingCart>("movies");
