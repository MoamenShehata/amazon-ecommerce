    ...
    <nav className="mt-4">
      <ul className="pagination justify-content-center">
        <li className={hasPreviousPage ? "page-item" : "page-item disabled"}>
          <button
            className={hasPreviousPage ? "page-link" : "page-link disabled"}
        ...
        {getPageNumbers().map((pageNum) => {
          return (
            <li
              key={pageNum}
              className={
                pageNum === currentPageNumber ? "page-item active" : "page-item"
              }
            >
              {pageNum.toString() !== "..." && (
                <button
        ...
        <li className={hasNextPage ? "page-item" : "page-item disabled"}>
          <button
            className={hasNextPage ? "page-link" : "page-link disabled"}
            onClick={nextPage}
          >
      ...
      <div className="text-center text-muted mt-3">
        <small>
          Page {currentPageNumber} of {Math.ceil(totalCount / pageSize)} (Total:
          {totalCount} products)
        </small>
      </div>
    </nav>
