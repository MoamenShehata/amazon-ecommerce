...
export function ProductPreview({
  product,
  onDeleted,
}: Readonly<{ product: ProductForListModel; onDeleted: () => void }>) {
  function deleteProduct() {
    catalogServices.deleteProduct(product.id).then(
      (res) => {
        onDeleted();
      },
      (err) => {},
    ...
    <div className="col-lg-3 col-md-4 col-sm-6 mb-4">
      <div className="card">
        ...
        <div className="card-footer">
          ...
          {!product.isAvailable && (
            <h4 className="text-danger">Currently Unavailable</h4>
          )}
        </div>
      </div>
    </div>
